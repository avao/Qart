...
namespace Qart.Core.DataStore
{
    ...
    public static class DataStorageExtensions
    {
        ...
        public static async Task<string> GetContentAsync(this IDataStore dataStore, string itemId)
        {
            if (!dataStore.Contains(itemId))
                return null;
        ...
        public static async Task PutContentAsync(this IDataStore dataStore, string itemId, string content)
        {
            using var stream = dataStore.GetWriteStream(itemId);
            using var writer = new StreamWriter(stream);
